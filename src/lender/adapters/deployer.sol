...
contract AdapterDeployer {
    ...
    function deployClerk(address lenderDeployer_, bool wireReserveAssessor)
        public
    {
        require(
            deployUsr == msg.sender &&
                address(clerk) == address(0) &&
                LenderDeployerLike(lenderDeployer_).seniorToken() != address(0)
        );

        lenderDeployer = LenderDeployerLike(lenderDeployer_);
        clerk = clerkFab.newClerk(
            lenderDeployer.currency(),
    ...
    function wireClerk(
        ...
        MemberlistLike(lenderDeployer.seniorMemberlist()).updateMember(
            mgr_,
            type(uint256).max
        );
    }
